...
let
  opencodeCodexAuthInstall = pkgs.writeShellApplication {
    name = "opencode-codex-auth-install";
    runtimeInputs = [ pkgs.nodejs ];
    text = ''
      exec npm exec --yes --package opencode-openai-codex-auth@4.4.0 -- opencode-openai-codex-auth "$@"
    '';
  };

  opencodeCodexAuthImport = pkgs.writeShellApplication {
...
in
pkgs.buildEnv {
  name = "opencode-codex-auth-tools";
  paths = [
    opencodeCodexAuthInstall
    opencodeCodexAuthImport
  ];
}
